...
st.write(f'mean rating is {no_of_rated_products_per_user}')
...
st.write(f'mean rating is {rating_mean_count}')

rating_mean_count['rating_counts'] = pd.DataFrame(new_df.groupby('product_id')['rating'].count())
st.write(rating_mean_count.sort_values(by='rating_counts', ascending=False))
...
st.title('----------------------------------------------------------------------------------------------------')
st.title('Item based Recommendation')
...
product_descriptions = pd.read_csv('data/product_descriptions.csv.zip')
